...
def network(type, N, p=None, connections=None, save_as=None):
    ...
    plt.title(f"{type} Network with N={N}\nAvg degrees: {mean_degrees}, Avg clustering coeff: {mean_clustering}, Avg path length: {avg_path_length}")
    ...
    if save_as:
        ...
        plt.savefig(f"{type}/graph.png", format='png')
        print(f"Graph saved in {type}/Graph.png")
    # plt.show()
    ...
    savename1 = "Degree Distribution.png"
    ...
    plt.ylabel("Frequency")
    savename2 = "Clustering Coefficient.png"
    plt.savefig(f"{type}/{savename2}", format='png')
    print(f"Graph saved as {type}/{savename2}")
    # plt.show()

# Example usage
network('Erdos-Renyi', N=250, p=0.01)
network('Ring-Lattice', N=250, connections=4)
network('Small-World', N=250, p=0.1, connections=4)
network('Scale-Free', N=250, connections=2)